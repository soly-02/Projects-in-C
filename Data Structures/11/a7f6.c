...
int price;
                ...
                /*τύπος δεδομένων για τα στοιχεία της λίστας
   ...
   int i, SubIndex;
     ...
     printf("========================================\n");
          ...
          for (i=0; i<HMax;i++)
          {
              ...
              {
                   printf("[%d, %s, %d, %d]",HList.List[SubIndex].RecKey,HList.List[SubIndex].Data.username,HList.List[SubIndex].Data.price,HList.List[SubIndex].Link);
                   printf(" -> ");
...
void PrintPinakes(HashListType HList)
{
    ...
    for (i=0;i<HList.Size;i++)
	   printf("%d) %d, %s, %d, %d\n",i,HList.List[i].RecKey,HList.List[i].Data.username,HList.List[i].Data.price,HList.List[i].Link);
	printf("\n");
}
...
scanf("%s",&AnItem.Data.username);
        printf("Give amount: ");
scanf("%d",&AnItem.Data.price);
 AnItem.RecKey=AnItem.Data.code;
 AnItem.Link=-1;
 AddRec(&HList,AnItem);
...
SearchHashList(HList,AnItem.Data.code,&Loc,&Pred);
if (Loc!=-1)
  printf("[%d, %s, %d, %d]\n",HList.List[Loc].RecKey,HList.List[Loc].Data.username,HList.List[Loc].Data.price,HList.List[Loc].Link);
else
 printf ("DEN YPARXEI EGGRAFH ME KLEIDI %d\n",AnItem.Data.code);
...
printf("Give amount: ");
scanf("%d",&AnItem.Data.price);
if (HList.List[Loc].Data.price>=AnItem.Data.price){
   HList.List[Loc].Data.price=HList.List[Loc].Data.price-AnItem.Data.price;
   }
...
SearchHashList(HList,AnItem.Data.code,&Loc,&Pred);
if (Loc!=-1){
if (HList.List[Loc].Data.price==0){
    DeleteRec(&HList,AnItem.Data.code);
...
if (Loc!=-1)
  {
printf("[%d, %s, %d, %d]\n",HList.List[Loc].RecKey,HList.List[Loc].Data.username,HList.List[Loc].Data.price,HList.List[Loc].Link);
PrintListOfSynonyms(HList,AnItem.Data.code);
}
else
 printf ("DEN YPARXEI EGGRAFH ME KLEIDI %d\n",AnItem.Data.code);
...
void PrintListOfSynonyms(HashListType HList, KeyType key){
int Hval, SubIndex,Pred;

//Διασχίζουμε τη λίστα συνωνύμων για το key

SubIndex = HList.HashTable[HashKey(key)];


while ( SubIndex != EndOfList )
              {
                   printf("%d: [%d, %s, %d]\n",SubIndex, HList.List[SubIndex].RecKey,HList.List[SubIndex].Data.username,HList.List[SubIndex].Data.price);
                   SubIndex = HList.List[SubIndex].Link;
